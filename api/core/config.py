...
try:
    ...
    def SettingsConfigDict(**kwargs: Any) -> dict:  # type: ignore[no-redef]
        return kwargs
...
class Settings(BaseSettings):
    PROJECT_NAME: str = "Prayog Government Innovation Platform"
...
def get_supabase_client() -> Optional[Client]:
    """
    Returns Supabase client if installed and configured, otherwise None.
    """
...
def get_supabase_admin_client() -> Optional[Client]:
    """
    Returns Supabase admin client if installed and configured, otherwise None.
    """
    if create_client is None:
        return None
    settings = get_settings()
